...
| Sandbox tournament creation | Implemented locally as public third tournament type using real CHPP team metadata and normal tournament mechanics; creation now cleans up partial rows after dependent insert failures, reports a clear migration error, and bypasses verified-team/archive rules | `053`; pending Supabase application | `npm run build` and `npm test` passed 2026-07-13 | Apply `053` and confirm |
