...
`npm run build` passed on 2026-07-14. `npm test` ran 68 tests with 67 passing and one unrelated existing world-details flag assertion failure (`💃🏻` vs `🇺🇳`). The build still reports the existing CSS `:global` warning and large bundle warning. For future code changes, run:
