...
| APPG scoring and result entry | Implemented locally for organizer-managed and sandbox tournaments. APPG outcomes are explicit (`ET3`, `ET2`, `PS1`, `RT0`, `OPW`, or `needs_review`); real tournaments use bulk result editing and CSV import, while sandbox tournaments can random-fill a round or season. APPG averages use classified results only. Newly added, manually linked, and live-refreshed finished APPG matches now use structured CHPP goal evidence for conservative auto-classification; organizer and CSV decisions remain authoritative, and existing `needs_review` rows are retried once per tournament session | `060_add_appg_match_outcomes.sql`; migration file created, Supabase application not verified | `npm run lint`, `npm test` (112), `npm run build`, server import check, 12-function count, and `git diff --check` passed 2026-07-22 | Apply `060`, then test a disposable organizer-managed APPG tournament with real Supabase data and verify auto-classification for add, link, refresh, and existing review rows |
| No pre-made schedule / Add HT match | Implemented locally. Admin Settings now uses a Generated schedule / No pre-made schedule dropdown. Generated keeps the existing round-robin flow; No pre-made shows suggested Hattrick matches first, can scan up to 3 teams for small tournaments or one selected team for larger tournaments, and still keeps the manual single Match ID fetch/preview/add flow behind a text link. Adding a suggested or manual match creates one normal fixture after both teams are registered and the admin confirms; the admin panel refreshes fixtures only, not the whole tournament view | `065_allow_manual_schedule_mode.sql` finalizes the earlier observed-schedule prototype by converting existing `observed` tournament rows to `manual` and setting the final schedule-mode constraint; match rows use existing fields from earlier fixture, APPG, structured-event, and observed-prototype migrations | `npm run build`, `npm test` (107), `npm run lint`, 12-function count, and `git diff --check` passed 2026-07-21 | Apply `065`, set a disposable tournament to No pre-made schedule, fetch suggested matches between registered teams, then add one real friendly match ID involving two registered teams |
...
`npm run check:server-imports`, `npm run lint`, `npm test` (112), `npm run build`, the 12-function count, and `git diff --check` passed on 2026-07-22 for structured APPG CHPP classification, result-entry cleanup, and the existing tournament workflows. The server import check scans `api/` and `shared/` and requires explicit runtime `.js` extensions for relative server imports. The counted function total remains `12` after consolidating presence, history, activity, Forge session, and Forge statistics behind `api/app.ts`, while CHPP testing remains consolidated behind `api/testing/index.ts`. Live Supabase migration/application, CHPP response verification, and Vercel verification remain outstanding.
