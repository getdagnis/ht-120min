...
| Sandbox tournament creation | Implemented locally as public third tournament type using real CHPP team metadata and normal tournament mechanics; creation now cleans up partial rows after dependent insert failures and reports a clear migration error | `053`; pending Supabase application | `npm run build` and `npm test` passed 2026-07-13 | Apply `053` and confirm |
