...
| Legacy tournament league type cleanup | Implemented locally; removed stale `league_type` state, hydration, dirty-state comparison, and update payload from `TournamentView`; `league_category` remains the canonical male/HFI field | No migration yet; drop the legacy column only after this code is deployed | `npm run build`, `npm run lint`, `npm test` (69), and `git diff --check` passed 2026-07-14 | Apply a follow-up migration to drop `tournaments.league_type` |
