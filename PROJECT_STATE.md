...
| Season-long tournament poll | Implemented locally in the standings sidebar. Ongoing seasons with a dated middle round show one low-stakes prediction vote per Hattrick manager, then read-only aggregate results after a vote or at the middle-round calendar date; planned and finished seasons do not render it | `068_tournament_season_poll_votes.sql` created locally; not applied | `tests/season-poll.test.ts`, `npm test` (186), lint, build, server-import, and diff checks passed locally; disposable Supabase/RLS execution was unavailable in this checkout | Apply `068` to a disposable Supabase project, verify anon read/insert plus duplicate-vote rejection, then deploy and test with two real Hattrick sessions |
...
- Some CHPP parsing remains ad hoc, especially around `src/server/api/teams/info.ts`.
