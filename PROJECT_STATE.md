...
| APPG scoring and result entry | Implemented locally for organizer-managed and sandbox tournaments. APPG outcomes are explicit (`ET3`, `ET2`, `PS1`, `RT0`, `OPW`, or `needs_review`); real tournaments use bulk result editing and CSV import, while sandbox tournaments can random-fill a round or season. APPG averages use classified results only, and match linking refreshes fixtures/standings without reloading the whole tournament view. Existing CHPP results remain `needs_review` until event evidence is classified | `060_add_appg_match_outcomes.sql`; migration file created, Supabase application not verified | `npm run lint`, `npm test` (104), `npm run build`, server import check, 12-function count, and `git diff --check` passed 2026-07-20 | Apply `060`, then test a disposable organizer-managed APPG tournament with real Supabase data |
