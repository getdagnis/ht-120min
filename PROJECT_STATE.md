...
| APPG scoring and result entry | Implemented locally for organizer-managed and sandbox tournaments. Persisted `scoring_mode = 'appg'` now resolves to the internal APPG-120 profile, separating average-points aggregation from the English 120-minute ET3/ET2/PS1/RT0/OPW ruleset. APPG outcomes remain explicit (`ET3`, `ET2`, `PS1`, `RT0`, `OPW`, or `needs_review`); real tournaments use bulk result editing and CSV import, while sandbox tournaments can random-fill a round or season. Newly added, manually linked, and live-refreshed finished APPG-120 matches use structured CHPP goal evidence for conservative auto-classification; organizer and CSV decisions remain authoritative. APPG history awards use completed-match eligibility and replace fixture-completion with most matches played; injury awards preserve counts while ranking by structured injury weeks when available | `060_add_appg_match_outcomes.sql`; migration file created, Supabase application not verified | `npm run lint`, `npm test` (121), `npm run build`, server import check, 12-function count, and `git diff --check` passed 2026-07-22 | Apply `060`, then test a disposable organizer-managed APPG-120 tournament with real Supabase data and verify classification, unequal-match history awards, injury-week awards, add, link, refresh, and existing review rows |
