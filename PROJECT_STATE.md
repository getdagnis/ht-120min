...
- Standings now has a text-only visual scoring toggle for switching between the 120-minute and regular 90-minute column views without changing tournament data. The visible rows resort to the selected mode's natural ranking, and each visible metric header supports ascending/descending sorting.
...
Latest code validation for tournament seasons/history and yearbook:
