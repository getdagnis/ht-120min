...
export function useLiveMatches(tournamentId: string | undefined, matches: any[], onMatchFinished?: () => void) {
        ...
        if (res.ok) {
          ...
          setLiveData(prev => {
            ...
            Object.entries(data.results as Record<string, LiveMatchData>).forEach(([id, result]) => {
               if (!next[id] || next[id].homeGoals !== result.homeGoals || next[id].awayGoals !== result.awayGoals || next[id].status !== result.status) {
                  next[id] = result;
