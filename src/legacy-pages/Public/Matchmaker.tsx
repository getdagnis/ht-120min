...
export const Matchmaker: React.FC = () => {
  ...
  useEffect(() => {
    if (process.env.NODE_ENV === 'production' || !isSelectingTeam) return;
    console.table(
  ...
  useEffect(() => {
    if (process.env.NODE_ENV === 'production' || scoredRequests.length === 0) return;
  ...
  useEffect(() => {
    if (process.env.NODE_ENV === 'production' || !selectedTeam) return;
    console.table([
      {
