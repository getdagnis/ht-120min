...
interface MatchWithTeams {
  ...
  completed: boolean;
  went_120: boolean;
  home_team: { name: string; ht_team_id: number };
  away_team: { name: string; ht_team_id: number };
}
...
export const TournamentView: React.FC = () => {
  ...
  const fetchData = React.useCallback(async () => {
    ...
    if (tournamentData) {
      ...
      const { data: teamsData } = await supabase.from('teams').select('*').eq('tournament_id', tournamentData.id);
      ...
      if (teamsData) {
        ...
        const calculated = calculateStandings(
          ...
          matchesWithTeams,
          tournamentData.scoring_mode as '120m' | 'points',
        );
  ...
  const handleJoin = () => {
    if (tournament) window.location.href = `/api/auth/init?tournamentId=${tournament.id}`;
  };
      ...
      <header className={styles.header}>
        ...
        <div className={styles.description}>
          {tournament.scoring_mode === '120m' ? (
            <p>
              <strong>120m Training Mode:</strong> Teams compete primarily on matches where 120 minutes of training was
              achieved. Standings are ranked by <strong>120m Achievements</strong>. Ties are settled by standard victory
